...
import { updatePairDayData, updateTokenDayData, updateXligerDayData, updatePairHourData } from "./dayUpdates";
import { getBnbPriceInUSD, findBnbPerToken, getTrackedVolumeUSD, getTrackedLiquidityUSD } from "./pricing";
import { convertTokenToDecimal, ADDRESS_ZERO, XFACTORY_ADDRESS, ONE_BI, ZERO_BD, BI_18 } from "./utils";
...
export function handleSync(event: Sync): void {
  ...
  let xligerData = XligerFactory.load(XFACTORY_ADDRESS);

  // reset factory liquidity by subtracting only tracked liquidity
  xligerData.totalLiquidityBNB = xligerData.totalLiquidityBNB.minus(pair.trackedReserveBNB as BigDecimal);
  ...
  xligerData.totalLiquidityBNB = xligerData.totalLiquidityBNB.plus(trackedLiquidityBNB);
  xligerData.totalLiquidityUSD = xligerData.totalLiquidityBNB.times(bundle.bnbPrice);
...
export function handleMint(event: Mint): void {
  ...
  xligerData.totalTransactions = xligerData.totalTransactions.plus(ONE_BI);
...
export function handleSwap(event: Swap): void {
  ...
  let xligerData = XligerFactory.load(XFACTORY_ADDRESS);
  xligerData.totalVolumeUSD = xligerData.totalVolumeUSD.plus(trackedAmountUSD);
  xligerData.totalVolumeBNB = xligerData.totalVolumeBNB.plus(trackedAmountBNB);
  xligerData.untrackedVolumeUSD = xligerData.untrackedVolumeUSD.plus(derivedAmountUSD);
  xligerData.totalTransactions = xligerData.totalTransactions.plus(ONE_BI);
  ...
  let xligerDayData = updateXligerDayData(event);
  let token0DayData = updateTokenDayData(token0 as Token, event);
  let token1DayData = updateTokenDayData(token1 as Token, event);

  // swap specific updating
  xligerDayData.dailyVolumeUSD = xligerDayData.dailyVolumeUSD.plus(trackedAmountUSD);
  xligerDayData.dailyVolumeBNB = xligerDayData.dailyVolumeBNB.plus(trackedAmountBNB);
  xligerDayData.dailyVolumeUntracked = xligerDayData.dailyVolumeUntracked.plus(derivedAmountUSD);
  xligerDayData.save();
